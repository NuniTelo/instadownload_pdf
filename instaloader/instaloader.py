...
class Instaloader:
    ...
    def download_post(self, post: Post, target: Union[str, Path]) -> bool:
        ...
        fn =  dirname + '/' + self.format_filename(post, target=target)
        ...
        self.create_pdf(filename) #create pdf
        self.context.log()
        return downloaded
    ...
    def create_pdf(self,filename):
        f = None
        try:
            f = open(filename+ ".txt", "r")  #here we have the description we need
            file_content =f.read()
            f.close()
        except IOError:
            file_content = "NO DESCP"
            

        pdf.add_page()
        try:
            pdf.image(filename + ".jpg", x=56, y=30, w=100)
        except:
            pdf.image("deafaultImages/iswanto-arif-442407-unsplash.jpg", x=56, y=30, w=100)
        ...
        for x in file_data:
            pdf.cell(600, 8, txt="{}".format(x.encode('utf-8')), ln=1)
                    ...
                    for post in profile.get_posts():
                        ...
                        with self.context.error_catcher("Download {} of {}".format(post, profile_name)):
                            ...
                            if fast_update and not downloaded and not post_changed:
                                break
                                #This is the line we call pdf creator so we can generate the pdf now with images and text
                    pdf.output(profile_name + ".pdf") 
                    self.context.log("Retrieving PRINTIINNNGGGGGG DONEEEE BITCCHHH " + profile_name) 
